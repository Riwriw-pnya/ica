...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 0Sub-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" />
        </svg>
      ),
    },
    {
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                href={item.href}
                className={`relative flex items-center justify-between px-3.5 py-3 rounded-2xl text-sm font-semibold transition-all ${
                  isActive
                    ? "bg-gradient-to-r from-[#FFF0E5] to-[#FFF9F5] text-[#C25115] shadow-sm border-l-4 border-[#EE6B28]"
                    : "text-[#4A3E36] hover:bg-[#FAF8F5] hover:text-[#231A14]"
                }`}
              >
                ...
                {!isCollapsed && item.badge && (
                  <span className="bg-[#FAF8F5] border border-[#EFE9E1] text-[#8C8078] text-[11px] px-2 py-0.5 rounded-full font-bold ml-2">
                    {item.badge}
                  </span>
                )}
              </Link>
      ...
      <div className="p-3 border-t border-[#F2EFE9] flex-shrink-0">
        <div className="flex items-center justify-between p-2 rounded-2xl hover:bg-[#FAF8F5] transition cursor-pointer">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 rounded-full bg-[#FCE8D5] text-[#C25115] flex items-center justify-center font-bold text-xs flex-shrink-0">
              RN
            </div>
            {!isCollapsed && (
          ...
          {!isCollapsed && (
            <svg className="w-4 h-4 text-[#8C8078] flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
